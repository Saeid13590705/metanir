...
def report_health_index(df):
    # شاخص سلامت ترکیبی: سن (وزن 0.6)، توان (وزن 0.2)، سازنده (وزن 0.2)
    df['health_score'] = 100 - (df['sen_trans'] / 60 * 60)  # نمره سن
...
def report_investment_planning(df):
    df['replacement_cost'] = df['power'] * 10000
...
def create_histogram(data, title, color='#c9a84c'):
    ...
    fig.update_layout(
        title={'text': title, 'font': {'family': 'Vazirmatn', 'size': 16}},
        xaxis_title="مقدار",
        yaxis_title="تعداد",
        height=300,
...
def create_donut(values, labels, title, colors=None):
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        textfont=dict(family='Vazirmatn', size=12, color='#f0e6d0')
    )])
    fig.update_layout(
        title={'text': title, 'font': {'family': 'Vazirmatn', 'size': 16}},
        height=300,
        ...
        font=dict(family='Vazirmatn', color='#f0e6d0'),
        showlegend=True,
        legend=dict(font=dict(family='Vazirmatn', size=11))
    )
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.uploaded:
            ...
            if st.button("📤 خروج از فایل", use_container_width=True):
                st.session_state.uploaded = False
                st.rerun()
    ...
    with search_col1:
        search_term = st.text_input("", placeholder="جستجو بر اساس نام پست، منطقه، سازنده، سن یا توان...", key="search_main")
    with search_col2:
        if st.button("🔍 جستجو", use_container_width=True):
            pass
    if search_term:
        mask = pd.Series(False, index=df.index)
        for col in ['post_name', 'Umur', 'sathe_voltage', 'karkhaneh_transe', 'trans_no', 'type_tap_changer']:
            if col in df.columns:
                mask = mask | df[col].astype(str).str.lower().str.contains(search_term.lower(), na=False)
        if search_term.isdigit():
            num = int(search_term)
            if 'sen_trans' in df.columns:
                mask = mask | (df['sen_trans'] == num)
            if 'power' in df.columns:
                mask = mask | (df['power'] == num)
        search_results = df[mask]
        if len(search_results) > 0:
            st.success(f"✅ {len(search_results)} نتیجه پیدا شد")
            display_cols = ['post_name', 'Umur', 'sathe_voltage', 'sen_trans', 'power', 'karkhaneh_transe']
    ...
    with main_tabs[4]:
        st.markdown("## 📈 ۱۰ گزارش تخصصی بر اساس استانداردهای بین‌المللی")
        ...
        with sub_tabs[0]:
            ...
            <div class="metric-card">
                ...
                <div class="metric-label">بیشترین تعداد در گروه {dist.idxmax()}</div>
                <div class="metric-sub">طبق استاندارد IEC 60076</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(pd.DataFrame({'گروه سنی': dist.index, 'تعداد': dist.values}), use_container_width=True)
            fig = create_bar_chart(dist.index, dist.values, 'توزیع سنی بر اساس IEC', 'گروه سنی', 'تعداد', '#c9a84c')
            st.plotly_chart(fig, use_container_width=True, key="age_dist")

        # گزارش ۲
        with sub_tabs[1]:
            avg_health = df_reports['health_index'].mean()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">💚</span>
                <div class="metric-value">{avg_health:.1f}</div>
                <div class="metric-label">میانگین شاخص سلامت</div>
                <div class="metric-sub">بر اساس ISO 55001</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'health_index']].head(20), use_container_width=True)
            fig = create_histogram(df_reports['health_index'].dropna(), 'توزیع شاخص سلامت', '#28a745')
        ...
        with sub_tabs[2]:
            ...
            <div class="metric-card">
                ...
                <div class="metric-label">میانگین امتیاز ریسک</div>
                <div class="metric-sub">بر اساس CIGRE</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'probability', 'impact', 'risk_score']].head(20), use_container_width=True)
            top_risk = df_reports.nlargest(10, 'risk_score')
            fig = create_bar_chart(top_risk['post_name'], top_risk['risk_score'], '۱۰ ترانس با بالاترین ریسک', 'ترانس', 'امتیاز ریسک', '#dc3545')
            st.plotly_chart(fig, use_container_width=True, key="risk_bar")

        # گزارش ۴
        with sub_tabs[3]:
            priority_counts = df_reports['priority'].value_counts()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">🔧</span>
                <div class="metric-value">{priority_counts.get('بالا', 0)}</div>
                <div class="metric-label">تعداد ترانس با اولویت بالا</div>
                <div class="metric-sub">بر اساس IEEE C57.140</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'priority_score', 'priority']].head(20), use_container_width=True)
            fig = create_donut(priority_counts.values, priority_counts.index, 'اولویت تعمیرات', ['#28a745', '#ffc107', '#dc3545'])
            st.plotly_chart(fig, use_container_width=True, key="priority_donut")

        # گزارش ۵
        with sub_tabs[4]:
            obs_counts = df_reports['obsolescence'].value_counts()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">🔄</span>
                <div class="metric-value">{obs_counts.get('بالا', 0)}</div>
                <div class="metric-label">ترانس با ریسک منسوخ‌شدگی بالا</div>
                <div class="metric-sub">بر اساس CIGRE</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'sen_trans', 'obsolescence']].head(20), use_container_width=True)
            fig = create_donut(obs_counts.values, obs_counts.index, 'سطح منسوخ‌شدگی', ['#28a745', '#ffc107', '#dc3545'])
            st.plotly_chart(fig, use_container_width=True, key="obs_donut")

        # گزارش ۶
        with sub_tabs[5]:
            avg_reliability = df_reports['reliability'].mean()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">📉</span>
                <div class="metric-value">{avg_reliability:.2f}</div>
                <div class="metric-label">میانگین قابلیت اطمینان</div>
                <div class="metric-sub">بر اساس IEEE</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # گزارش ۷
        with sub_tabs[6]:
            avg_eff = df_reports['efficiency'].mean()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">💡</span>
                <div class="metric-value">{avg_eff:.1f}%</div>
                <div class="metric-label">میانگین بازده انرژی</div>
                <div class="metric-sub">بر اساس IEC</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'power', 'losses', 'efficiency']].head(20), use_container_width=True)
            top_eff = df_reports.nlargest(10, 'efficiency')
            fig = create_bar_chart(top_eff['post_name'], top_eff['efficiency'], '۱۰ ترانس با بیشترین بازده', 'ترانس', 'بازده (%)', '#28a745')
            st.plotly_chart(fig, use_container_width=True, key="efficiency_bar")

        # گزارش ۸
        with sub_tabs[7]:
            avg_roi = df_reports['roi'].mean()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">💰</span>
                <div class="metric-value">{avg_roi:.1f}%</div>
                <div class="metric-label">میانگین ROI (بازگشت سرمایه)</div>
                <div class="metric-sub">بر اساس ISO 55001</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'replacement_cost', 'repair_cost', 'roi']].head(20), use_container_width=True)
            fig = create_histogram(df_reports['roi'].dropna(), 'توزیع ROI', '#c9a84c')
            st.plotly_chart(fig, use_container_width=True, key="roi_hist")

        # گزارش ۹
        with sub_tabs[8]:
            total_co2 = df_reports['co2_emission'].sum()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">🌍</span>
                <div class="metric-value">{total_co2:.1f} تن</div>
                <div class="metric-label">کل انتشار CO₂ در سال</div>
                <div class="metric-sub">بر اساس ISO 14001</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'sen_trans', 'co2_emission']].head(20), use_container_width=True)
            top_co2 = df_reports.nlargest(10, 'co2_emission')
            fig = create_bar_chart(top_co2['post_name'], top_co2['co2_emission'], '۱۰ ترانس با بیشترین انتشار CO₂', 'ترانس', 'تن CO₂ در سال', '#dc3545')
            st.plotly_chart(fig, use_container_width=True, key="co2_bar")

        # گزارش ۱۰
        with sub_tabs[9]:
            bench_counts = df_reports['benchmark'].value_counts()
            st.markdown(f"""
            <div class="metric-card">
                <span class="metric-icon">📋</span>
                <div class="metric-value">{bench_counts.get('بالاتر از میانگین', 0)}</div>
                <div class="metric-label">ترانس با سن بالاتر از میانگین صنعت</div>
                <div class="metric-sub">بر اساس CIGRE</div>
            </div>
            """, unsafe_allow_html=True)
            st.dataframe(df_reports[['post_name', 'sen_trans', 'benchmark', 'cigre_advice']].head(20), use_container_width=True)
            fig = create_donut(bench_counts.values, bench_counts.index, 'وضعیت نسبت به میانگین صنعت', ['#28a745', '#dc3545'])
            st.plotly_chart(fig, use_container_width=True, key="bench_donut")
            st.markdown("#### 📘 توصیه‌های CIGRE")
